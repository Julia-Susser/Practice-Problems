...
bool permutations(int final[], int indx, int leftover[], int leftoverSize, vector<int *> &Permutations){
  //Arrays are immediately a reference wheras vectors are not.
  if (leftoverSize == 0){
    int array[indx];
    copyArray(array, final,indx);
    printArray(array,indx);
...
void PrintPermutations(vector<int *> Permutations, int size){
